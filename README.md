...
If an update ever fails with *"The data couldn't be read because it isn't in
the correct format"*, that one message covers everything from a malformed
manifest to a download that came back short, and tells you which of them it was:
nothing. Pull down to refresh the source in AltStore and try again - the
manifest is served with a five-minute cache, so a phone can briefly still be
looking at the previous one. CI now waits for the new manifest to actually be
served and checks every advertised download against the real asset before it
goes green, so the usual causes cannot leave the building.

The rule that keeps it that way: **one version, one tag, once**. Re-pushing a
tag replaces an asset that a published manifest is already describing, and a
phone handed a length that disagrees with the file fails with that exact
message.
